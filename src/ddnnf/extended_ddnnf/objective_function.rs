...
impl ObjectiveFn {
    pub fn eval(&self, var: u32, attrs: &HashMap<String, Attribute>) -> f64 {
        match self {
            ...
            Add(left_expr, right_expr) => {
                left_expr.eval(var, attrs) + right_expr.eval(var, attrs)
            },
            Sub(left_expr, right_expr) => {
                left_expr.eval(var, attrs) - right_expr.eval(var, attrs)
            },
            Mul(left_expr, right_expr) => {
                left_expr.eval(var, attrs) * right_expr.eval(var, attrs)
            },
            Div(left_expr, right_expr) => {
                left_expr.eval(var, attrs) / right_expr.eval(var, attrs)
            },
...
impl Condition {
    ...
    pub fn eval(&self, var: u32, attrs: &HashMap<String, Attribute>) -> bool {
        match self {
            And(left_cond, right_cond) => {
                left_cond.eval(var, attrs) && right_cond.eval(var, attrs)
            },
            Or(left_cond, right_cond) => {
                left_cond.eval(var, attrs) || right_cond.eval(var, attrs)
            },
            Xor(left_cond, right_cond) => {
                left_cond.eval(var, attrs) ^ right_cond.eval(var, attrs)
            },
...
pub fn get_numeric_attr_val_or_panic(attr_name: &str, var: u32, attrs: &HashMap<String, Attribute>) -> f64 {
    match attrs.get(attr_name) {
        Some(IntegerAttr(attr)) => {
            *attr.get_val(var).unwrap_or_else(|| panic!("No \'Integer\' value provided for feature {var}.")) as f64
        },
        Some(FloatAttr(attr)) => {
            *attr.get_val(var).unwrap_or_else(|| panic!("No \'Float\' value provided for feature {var}."))
        },
        Some(_) => panic!("Attribute {attr_name} is neither of type \'Integer\', nor of type \'Float\'."),
        None => panic!("Attribute {attr_name} does not exist.")
    }
}

pub fn get_bool_attr_val_or_panic(attr_name: &str, var: u32, attrs: &HashMap<String, Attribute>) -> bool {
    match attrs.get(attr_name) {
        Some(BoolAttr(attr)) => {
            *attr.get_val(var).unwrap_or_else(|| panic!("No \'Bool\' value provided for feature {var}."))
        },
...
pub fn get_string_attr_val_or_panic<'a>(attr_name: &str, var: u32, attrs: &'a HashMap<String, Attribute>) -> &'a String {
    match attrs.get(attr_name) {
        Some(StringAttr(attr)) => {
            attr.get_val(var).unwrap_or_else(|| panic!("No \'String\' value provided for feature {var}."))
        },
