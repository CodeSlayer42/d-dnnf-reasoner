...
impl OptimalConfig {
    ...
    pub fn unify_disjoint(mut self, other: &Self) -> Self {
        debug_assert!(
            self.config.get_decided_literals().all(
                |literal| !other.config.get_decided_literals().contains(&literal)
            )
        );
    ...
    pub fn calc_best_config_for_node_helper(&self, node_id: usize, assumptions: &[i32], partial_configs: &Vec<Option<OptimalConfig>>) -> Option<OptimalConfig> {
        ...
        match &node.ntype {
            ...
            And { children } => {
                ...
                if children_configs.iter().any(|child_config_opt| child_config_opt.is_none()) {
                    return None;
                }

                let unified_config = children_configs.into_iter()
                    .flatten()
                    .fold(
                        OptimalConfig::empty(number_of_variables),
                        |acc, next| acc.unify_disjoint(next)
                    );
    ...
    pub fn calc_top_k_configs_for_node_helper(&self, node_id: usize, k: usize, assumptions: &[i32], partial_configs: &Vec<Vec<OptimalConfig>>) -> Vec<OptimalConfig> {
        ...
        match &node.ntype {
            ...
            And { children } => {
                let children_configs = children.iter()
                    .map(|&child_node_id| {
                        partial_configs.get(child_node_id).unwrap_or_else(|| panic!("No partial configs for node {child_node_id} present."))
                    })
                    .collect_vec();

                if children_configs.iter().any(|child_configs| child_configs.is_empty()) {
                    return vec![];
                }
            ...
            Or { children } => {
                ...
                merge_top_k_results_or(children_configs, Some(k)).into_iter()
                    .cloned()
                    .collect_vec()
            },
        }
...
pub fn merge_top_k_results_and(sorted_lists: Vec<&Vec<OptimalConfig>>, number_of_variables: usize, max_amount: Option<usize>) -> Vec<OptimalConfig> {

    let insert_new_candidate = |idx_tuple: Vec<usize>, candidates_heap: &mut BinaryHeap<OptimalConfig>, candidate_idx_mapping: &mut BiHashMap<OptimalConfig, Vec<usize>>| {
        let candidate = sorted_lists.iter()
            .zip(idx_tuple.iter())
            .map(|(list, &i)| &list[i])
            .fold(
          OptimalConfig::empty(number_of_variables),
    ...
    for _ in 0..result_amount {
        ...
        let best_elem = candidates_heap.pop().expect("There must be candidates left.");
        let best_elem_idx_tuple = candidate_idx_mapping.get_by_left(&best_elem).expect("This index tuple must have been inserted.");
        ordered_cartesian_product.push(best_elem);

        let new_candidates = (0..sorted_lists.len())
            .filter(|&list_idx| {
                best_elem_idx_tuple[list_idx] + 1 < sorted_lists[list_idx].len()
            })
            .map(|list_idx| {
        ...
        new_candidates.into_iter().for_each(|idx_tuple| insert_new_candidate(idx_tuple, &mut candidates_heap, &mut candidate_idx_mapping));
    }
...
pub fn merge_top_k_results_or(sorted_lists: Vec<&Vec<OptimalConfig>>, max_amount: Option<usize>) -> Vec<&OptimalConfig> {
    ...
    for _ in 0..result_amount {
        let (best_elem, list_idx) = sorted_lists.iter().enumerate()
